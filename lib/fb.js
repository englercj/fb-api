...
FogBugz.prototype.getUserId = function(email, cb) {
	...
	cb(null, data.people[0].person[0].ixPerson[0].$.text());
    });
};
...
FogBugz.prototype.editCase = function(caseId, body, editedById, cb) {
    var args = {}, self = this;
...
FogBugz.prototype.email = function(caseId, from, to, subject, body, editedById, cb) {
    var args = {}, self = this;
...
FogBugz.prototype._req = function(u, cb) {
	...
	if(!err && res.statusCode == 200) {
	    ...
	    if(data.error && data.error.length && cb) cb(new Error('Error (' + data.error.$code + '): ' + data.error[0].$.text()));
...
FogBugz.prototype._getApiUrl = function(cb) {
	...
	self._version = data.version[0].$.text();
	self._minversion = data.minversion[0].$.text();
	self._url = self._url += data.url[0].$.text();
